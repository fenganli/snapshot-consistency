...
int main(int argc, char** argv) {
    ...
    std::vector<double> local_backup_time; 
	...
	for (int j = 0; j < insert_size; j++) {
	  ...
	  passed_time = ((double)BILLION*read_end.tv_sec+read_end.tv_nsec-
		(double)BILLION*read_start.tv_sec-read_start.tv_nsec)*1.0/BILLION;
          ...
          passed_time = ((double)BILLION*backup_end.tv_sec+backup_end.tv_nsec-
		(double)BILLION*backup_start.tv_sec-backup_start.tv_nsec)*1.0/BILLION;
          backup_time.push_back(passed_time);


	  // Now measure the local back up time
          struct timespec local_backup_start, local_backup_end;
          clock_gettime(CLOCK_MONOTONIC, &local_backup_start);
          result = exec("mongodump --oplog --quiet");
          std::cout << result << std::endl;
          clock_gettime(CLOCK_MONOTONIC, &local_backup_end);
          passed_time = ((double)BILLION*local_backup_end.tv_sec+local_backup_end.tv_nsec-
		(double)BILLION*local_backup_start.tv_sec-local_backup_start.tv_nsec)*1.0/BILLION;
          local_backup_time.push_back(passed_time); 
           
          db["restaurants"].drop(); // Drop the table 
    }
    std::cout << "Insert time: k MB, sec" << std::endl;
    for (int i = 0; i <= max_insert_size; i++)
        std::cout << insert_time[i] << std::endl;
    std::cout << "Read time: k MB, sec" << std::endl;
    for (int i = 0; i <= max_insert_size; i++)
        std::cout << read_time[i] << std::endl;
    std::cout << "Backup time: k MB, sec" << std::endl;
    for (int i = 0; i <= max_insert_size; i++)
	std::cout << backup_time[i] << std::endl;
    std::cout << "Local backup time: k MB, sec" << std::endl;
    for (int i = 0; i <= max_insert_size; i++)
	std::cout << local_backup_time[i] << std::endl; 
    return 0;
}
